#!/usr/bin/env bash
#=============================================================================
# Lab 12: Cambiar a LEGACY
# Habilita la crypto-policy LEGACY para máxima compatibilidad
#
...
print_header "Lab 12: Cambiar a política LEGACY"
...
echo
print_warning "La política LEGACY permite criptografía débil para compatibilidad"
print_warning "  - TLS 1.0 y 1.1 permitidos"
print_warning "  - Cifrados débiles habilitados"
print_warning "  - Usar solo para pruebas o compatibilidad con sistemas heredados"
echo

read -p "¿Continuar con la política LEGACY? (s/N): " -n 1 -r
echo
if [[ ! ${REPLY} =~ ^[Ss]$ ]]; then
...
# Cambiar a LEGACY
print_info "Cambiando a la política LEGACY..."
if update-crypto-policies --set LEGACY; then
  print_success "Política establecida en LEGACY"
else
  print_error "No se pudo establecer la política LEGACY"
  exit 1
fi
...
echo
print_success "Cambiado a la política LEGACY"
echo
echo "Probar con:"
echo "  openssl ciphers -v"
echo "  ssh -Q cipher"
echo
print_warning "Recuerde restaurar la política DEFAULT cuando termine las pruebas:"
