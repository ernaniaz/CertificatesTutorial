#!/usr/bin/env bash
#=============================================================================
# Lab 12: Switch Future
# Enable FUTURE crypto-policy for maximum security
#
# Usage: ./switch-future.sh
...
print_header "Lab 12: Switch to FUTURE Policy"
...
echo
print_warning "FUTURE policy enforces strong cryptography"
print_warning "  - TLS 1.2+ with preference for 1.3"
print_warning "  - Strong ciphers only"
print_warning "  - Larger key sizes required"
print_warning "  - May break compatibility with older systems"
echo

read -p "Continue with FUTURE policy? (y/N): " -n 1 -r
echo
if [[ ! ${REPLY} =~ ^[Yy]$ ]]; then
...
# Switch to FUTURE
print_info "Switching to FUTURE policy..."
if update-crypto-policies --set FUTURE; then
  print_success "Policy set to FUTURE"
else
  print_error "Failed to set FUTURE policy"
  exit 1
fi
...
echo
print_success "Switched to FUTURE policy"
echo
echo "Test with:"
echo "  openssl ciphers -v"
echo "  ssh -Q cipher"
echo
print_warning "If you encounter compatibility issues, restore DEFAULT:"
