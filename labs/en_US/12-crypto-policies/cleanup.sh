...
if [[ ${RHEL_VERSION} -lt 8 ]]; then
  echo "Crypto-policies not applicable to RHEL ${RHEL_VERSION}"
  exit 0
fi
...
echo
print_info "Restoring DEFAULT policy..."

# Restore DEFAULT
if update-crypto-policies --set DEFAULT; then
  print_success "Policy restored to DEFAULT"
else
  print_error "Failed to restore DEFAULT policy"
  exit 1
fi
...
for service in ${SERVICES}; do
  if systemctl is-active "${service}" &>/dev/null; then
    ...
    if ! systemctl restart "${service}" 2>/dev/null; then
      echo "    (restart failed or not installed)"
    fi
  fi
done
...
echo
echo "System restored to DEFAULT crypto-policy"
