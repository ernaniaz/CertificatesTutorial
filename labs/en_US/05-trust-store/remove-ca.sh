...
#=============================================================================
# Lab 05: Remove CA
# Remove custom CA from system trust
#
# Usage: ./remove-ca.sh
...
print_header "Lab 05: Removing Custom CA from Trust"
...
# Remove CA from trust anchors
if [[ -f "${TRUST_ANCHOR}" ]]; then
  print_info "Removing CA from trust anchors..."
  rm -f "${TRUST_ANCHOR}"
  print_success "CA removed from ${TRUST_ANCHOR}"
else
  echo "CA not found in trust anchors (already removed?)"
fi
...
echo
echo "Custom CA is no longer trusted by the system"
