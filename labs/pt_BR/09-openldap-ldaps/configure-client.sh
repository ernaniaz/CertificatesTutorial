...
# Configura o cliente LDAP para conexões TLS
#
...
# Backup da configuração original do cliente
print_info "Fazendo backup da configuração do cliente..."
if [[ -f /etc/openldap/ldap.conf ]]; then
  ...
  echo "Nenhum ldap.conf existente, criando um novo"
fi

echo

# Configurar configurações TLS do cliente
print_info "Configurando configurações TLS do cliente..."

cat >> /etc/openldap/ldap.conf << 'EOF'

# Lab 09: Configuração TLS do Cliente
...
# Base DN (ajuste conforme necessário)
BASE dc=example,dc=com
EOF

print_success "Configurações TLS do cliente configuradas"
echo
...
echo
print_success "Configuração do cliente LDAP concluída"
echo
echo "Testar com:"
