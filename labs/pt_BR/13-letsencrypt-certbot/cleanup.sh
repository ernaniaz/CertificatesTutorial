...
#=============================================================================
# Lab 13: Limpeza
# Remove o certbot e os certificados
#
# Uso: ./cleanup.sh
...
# Confirmação
print_warning "Isso removerá certbot e todos os certificados."
...
if command -v certbot &>/dev/null && [[ -d /etc/letsencrypt/live ]]; then
  ...
  print_success "Certificados excluídos"
fi

echo

# Parar e desabilitar timer
print_info "Desabilitando timer do certbot..."
systemctl stop certbot-renew.timer 2>/dev/null || true
systemctl disable certbot-renew.timer 2>/dev/null || true
print_success "Timer desabilitado"

echo

# Remover pacote certbot
print_info "Removendo pacote certbot..."

dnf remove -y certbot python3-certbot-apache python3-certbot-nginx 2>/dev/null || true

print_success "Certbot removido"
echo

# Remover diretório Let's Encrypt
print_info "Removendo diretório Let's Encrypt..."
